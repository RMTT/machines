...
in {
  ...
  config = {
    ...
    networking.networkmanager = mkIf (!cfg.useNetworkd) { enable = true; };
    ...
    networking.nftables = {
      ...
      tables = listToAttrs (lists.imap0 (i: config: {
        name = "nixos-route";
        value = {
          family = "inet";
          enable = true;
    ...
    systemd.services = listToAttrs (lists.imap0 (i: config:
      let
        name = config.name;
      ...
      in {
        ...
        value = {
          ...
          '';
          script = ''
            ip rule add preference ${
              toString config.rule.priority
            } fwmark ${config.rule.fwmark} lookup ${config.table.name}

            ip -6 rule add preference ${
              toString config.rule.priority
            } fwmark ${config.rule.fwmark} lookup ${config.table.name}
    ...
    systemd.timers = listToAttrs (lists.imap0 (i: config:
